...
class HistoryLogger:
    ...
    def open_history_file(self):
        ...
        self.history_file = open(f"history/{timestamp}_{self.conversation_id}.txt", "a")

    def log_prompt_response(self, prompt, response):
        if not self.history_file:
            raise ConversationNotInitializedError("History file must be opened before logging prompt and response")
